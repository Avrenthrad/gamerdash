// One Piece Card Game — My Collection hub. Cards + Decks (no binders/
// set-lists — not surfaced for this game, matching Pokémon's own
// home-page feature scope). Real card data via optcgapi.com.

import { useEffect, useState } from "react";
import { fetchCollection, enrichCollectionEntry, updateCollectionEntry, removeFromCollection, fetchDecks } from "../lib/onepieceCollection";
...
function cardPrice(entry) {
  return entry.card ? currentOnePiecePrice(entry.card)?.price || 0 : 0;
}

export default function OnePieceCollectionPage({ onBack, userId, onGoToSearch, onGoToDecks }) {
  ...
  return (
    <div className="price-page">
      <div className="price-page__head">
        ...
        <p className="price-page__subtitle">Real owned cards and decks — via optcgapi.com.</p>
      </div>
...
function CardsTab({ userId, onGoToSearch }) {
  ...
  async function loadCollection() {
    ...
    try {
      ...
      console.error("Failed to load One Piece collection:", err);
      setStatus("error");
    }
  }
...
// Compact deck list — the full editor lives in OnePieceDeckBuilderPage.
        ...
        <div className="binder-grid">
          {decks.map((deck) => (
            <button key={deck.id} type="button" className="binder-card binder-card--plain" onClick={onGoToDecks}>
              ...
              <span className="binder-card__meta">{deck.leader_card_name || "No Leader yet"}</span>
